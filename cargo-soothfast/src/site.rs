...
use soothfast_site::{BuildInput, SiteConfig, build, config, evidence, search, serve};
...
fn assemble(root: &Path, baseline: &str, out: Option<&Path>) -> Result<BuildInput, String> {
...
pub fn build_cmd(baseline: &str, out: Option<&Path>) -> i32 {
    ...
    let input = match assemble(&root, baseline, out) {
        Ok(i) => i,
        Err(e) => return err(&e),
    };
    match build(&input) {
...
pub fn serve_cmd(baseline: &str, addr: Option<&str>) -> i32 {
    ...
    let input = match assemble(&root, baseline, Some(&serve_dir)) {
        Ok(i) => i,
        Err(e) => return err(&e),
    };
    if let Err(e) = build(&input) {
        return err(&e);
    }
    ...
    let result = server.run(&serve_dir, &watch, move || {
        let input = assemble(&root, &baseline, Some(&rebuild_dir))?;
        build(&input)
    });
    match result {
        Ok(()) => 0,
...
pub fn gh_deploy_cmd(baseline: &str, branch: &str, remote: &str, message: Option<&str>) -> i32 {
    ...
    let input = match assemble(&root, baseline, Some(&build_dir)) {
        Ok(i) => i,
        Err(e) => return err(&e),
    };
