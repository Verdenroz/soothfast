...
fn measure_ref_interleaved(common: &CommonArgs, refname: &str) -> Result<(Value, Run), String> {
    ...
    let (head1, base1, head2, base2) = invoke::with_merge_base_worktree(refname, |wt| {
        Ok((
            measure(None),
            measure(Some(wt)),
            measure(None),
