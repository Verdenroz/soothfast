...
fn python_files() -> std::collections::BTreeMap<String, String> {
    let (surface, gaps) = walk();
    BindKind::Python
...
#[test]
fn the_stub_lists_handles_properties_seqs_enums_and_exceptions_as_python_sees_them() {
    let files = python_files();
    assert_eq!(files["py.typed"], "");
    let stub = &files["acme_core.pyi"];
    assert!(stub.starts_with(
        "# Code generated by `cargo soothfast bind gen`. DO NOT EDIT.\n\nfrom collections.abc import Iterator\n\n"
    ), "{stub}");
    assert!(stub.contains(
        "class ClientSeq:\n    def __len__(self) -> int: ...\n    def __getitem__(self, index: int) -> Client: ...\n    def __iter__(self) -> Iterator[Client]: ...\n    def __repr__(self) -> str: ...\n    def tolist(self) -> list[Client]: ...\n    @property\n    def symbol(self) -> list[str]: ...\n    @property\n    def price(self) -> F64Array: ...\n    @property\n    def maybe(self) -> list[int | None]: ...\n\n"
    ), "{stub}");
    assert!(stub.contains(
        "    @property\n    def price(self) -> float: ...\n    @price.setter\n    def price(self, value: float) -> None: ...\n"
    ), "{stub}");
    assert!(stub.contains(
        "    @staticmethod\n    async def new(symbol: str) -> Client: ...\n    @staticmethod\n    def new_blocking(symbol: str) -> Client: ...\n"
    ), "{stub}");
    assert!(
        stub.contains(
            "    async def chart(self) -> int: ...\n    def chart_blocking(self) -> int: ...\n"
        ),
        "{stub}"
    );
    assert!(stub.contains(
        "class Bag:\n    @property\n    def items(self) -> ClientSeq: ...\n    @property\n    def level(self) -> Level | None: ...\n    @level.setter\n    def level(self, value: Level | None) -> None: ...\n    @property\n    def owner(self) -> Client: ...\n    def __repr__(self) -> str: ...\n\n"
    ), "{stub}");
    assert!(stub.contains(
        "class Level:\n    Low: Level\n    High: Level\n    def __eq__(self, other: object) -> bool: ...\n    def __int__(self) -> int: ...\n\n"
    ), "{stub}");
    assert!(stub.contains(
        "class Error(Exception):\n    ...\n\nclass Failure(Error):\n    ...\n\nclass NotFound(Failure):\n    symbol: str\n    retry: int | None\n\nclass Wrapped(Failure):\n    ...\n\nclass Busy(Failure):\n    ...\n\n"
    ), "{stub}");
    assert!(stub.contains("\ndef probe() -> bool: ...\n"), "{stub}");
    assert!(
...
#[test]
fn the_stub_spells_a_mapped_type_as_str_and_lists_a_free_blocking_twin() {
    let mut table = TypeTable::with_defaults();
    table.insert("chrono::DateTime", Ty::Text("chrono::DateTime".into()));
    let records = vec![
        record("acme::core::Client", "struct"),
    ...
    assert!(
        stub.contains("\ndef maybe_stamp() -> str | None: ...\n"),
        "{stub}"
    );
    assert!(stub.contains(
        "    @property\n    def clock(self) -> str: ...\n    @clock.setter\n    def clock(self, value: str) -> None: ...\n"
    ), "{stub}");

    let records = vec![record("acme::core::poll", "fn")];
    let (walked, gaps) = surface(
        &stamped(doc_with_sync_client_and_poll()),
        &TypeTable::with_defaults(),
        &records,
    ...
    let stub = &files.files["acme_core.pyi"];
    assert!(
        stub.contains("\nasync def poll() -> bool: ...\ndef poll_blocking() -> bool: ...\n"),
        "{stub}"
    );
    assert!(!stub.contains("from collections.abc"), "{stub}");
}
