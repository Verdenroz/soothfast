# acme.core

Kotlin bindings for the `acme` crate, generated by [soothfast](https://github.com/Verdenroz/soothfast). Do not edit by hand: run `cargo soothfast bind gen` instead.

## Build

```bash
cargo build --release
```

That writes `target/release/libacme_core.so` (`.dylib` on macOS, `acme_core.dll` on Windows). Compile the Kotlin sources against it and put the library on the loader's path:

```bash
kotlinc -d out $(find src/main/kotlin -name '*.kt')
jar --create --file acme_core.jar -C out .
java -Djava.library.path=target/release -cp acme_core.jar YourMainKt
```

## Use

```kotlin
import acme.core.Core
```

Every handle class implements `AutoCloseable`; call `close()` (or use Kotlin's `use { }`) to free it deterministically. A `java.lang.ref.Cleaner` frees one you forgot, but not on any schedule you should rely on.

A borrowed buffer parameter (`DoubleArray`, `ByteArray`, ...) is read through `GetPrimitiveArrayCritical`: it crosses without a copy, but the garbage collector cannot run for the length of that one call.

## Types

### `Counter`

### `Level`

### `Mode`

## `Core`

- `describe`
- `describeOwned`
- `digest`
- `fail`
- `findCounter`
- `flags`
- `greet`
- `invertBits`
- `isHigh`
- `mutateCounter`
- `normalize`
- `peakLevel`
- `sampleIds`
- `scaleInto`
- `split`
- `stamp`

## Not bound

- acme::Counter::consume: takes `self` by value, which would empty the handle the binding holds; take `&self` and return a new value instead
- acme::with_time: foreign type `chrono::DateTime` has no mapping; add one under [bind.types] in soothfast.toml
- acme::Counter::refresh: `async fn` cannot cross into kotlin: no Kotlin runtime story yet
- acme::counters: `Vec<Counter>` cannot cross into kotlin: a sequence of `Counter` has no C spelling that owns its elements; a sequence of one primitive crosses as a pointer and a length
- acme::index_all: `HashMap<String, u32>` cannot cross into kotlin: C has no map type; return a sequence of pairs, or an exported type with accessors
- acme::levels: `Vec<Level>` cannot cross into kotlin: a sequence of `Level` has no C spelling that owns its elements; a sequence of one primitive crosses as a pointer and a length
- acme::maybe_ratio: `Option<f64>` cannot cross into kotlin: `Option<f64>` has no C spelling; only an optional exported type or string does, as a pointer that may be null
- acme::merge: crosses the exported type `Counter` by value, which would copy it; take it by reference, or add a method returning what the caller needs
- acme::peek: `Option<Counter>` cannot cross into kotlin: an optional exported type is taken only as a return; take it by reference instead
- acme::scale_optional: `Option<f64>` cannot cross into kotlin: `Option<f64>` has no C spelling; only an optional exported type or string does, as a pointer that may be null
- acme::set_level: `Option<Level>` cannot cross into kotlin: an optional exported type is taken only as a return; take it by reference instead
- acme::sum_optional: `Option<Vec<f64>>` cannot cross into kotlin: `Option<Vec<f64>>` has no C spelling; only an optional exported type or string does, as a pointer that may be null
- acme::trim: `Option<Vec<f64>>` cannot cross into kotlin: `Option<Vec<f64>>` has no C spelling; only an optional exported type or string does, as a pointer that may be null
