...
#[repr(C)]
pub struct AcmeCoreUsizeArray {
    data: *mut usize,
    len: usize,
}

impl AcmeCoreUsizeArray {
    fn empty() -> Self {
        AcmeCoreUsizeArray { data: ::std::ptr::null_mut(), len: 0 }
    }

    fn new(values: Vec<usize>) -> Self {
        let mut boxed = values.into_boxed_slice();
        let out = AcmeCoreUsizeArray { data: boxed.as_mut_ptr(), len: boxed.len() };
        ::std::mem::forget(boxed);
        out
    }
}
...
/// did not return, is undefined.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn acme_core_usize_array_free(array: AcmeCoreUsizeArray) {
    if array.data.is_null() {
        return;
    }
    drop(unsafe {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn acme_core_counter_absorb(handle: *mut AcmeCoreCounter, other: *const AcmeCoreCounter) {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn acme_core_counter_scale(handle: *mut AcmeCoreCounter, factor: i64) -> i64 {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn acme_core_describe(label: *const ::std::os::raw::c_char) -> *mut ::std::os::raw::c_char {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn acme_core_sample_ids(ids: *const usize, ids_len: usize) -> AcmeCoreUsizeArray {
    AcmeCoreUsizeArray::new(::acme::sample_ids(unsafe { ffi::slice(ids, ids_len) }.to_vec()))
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn acme_core_scale_into(values: *const f64, values_len: usize, factor: f64, out: *mut f64, out_len: usize) {
