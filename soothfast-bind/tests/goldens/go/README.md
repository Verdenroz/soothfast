...
- acme::counters: `Vec<Counter>` cannot cross into go: a sequence of `Counter` has no C spelling that owns its elements; a sequence of one primitive crosses as a pointer and a length
...
- acme::levels: `Vec<Level>` cannot cross into go: a sequence of `Level` has no C spelling that owns its elements; a sequence of one primitive crosses as a pointer and a length
- acme::maybe_ratio: `Option<f64>` cannot cross into go: `Option<f64>` has no C spelling; only an optional exported type or string does, as a pointer that may be null
