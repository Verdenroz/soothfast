...
produced by this exact code, every CI run:
...
Docs and performance work the same way: written **from the code**, checked
against it on every build. One runtime dependency (`linkme`); everything
else lives in the separate `cargo-soothfast` binary.
...
`cargo soothfast measure` records perfcnt / callgrind / walltime / alloc
into a baseline. `cargo soothfast gate` fails CI the moment a fresh run
regresses past a threshold. `<!-- soothfast:bind -->` /
`<!-- soothfast:claim -->` markers tie markdown prose to that same run, so
a stale README is a CI failure, not a lucky accident.

There's more beyond benchmarks — spec reconciliation, trend charts,
changelog drafts, an agent-facing MCP server — covered in the
**[full guide](https://verdenroz.github.io/soothfast/)**.
...
CI runs soothfast on soothfast: `soothfast-registry`, `soothfast-measure`,
and `soothfast-docs` each carry a bench target measured into a fresh
baseline on every build, and this README is checked against it. These are
...
equivalent); without it LLVM const-folds the measured body away — the #1
measurement footgun.
<!-- /soothfast:bind -->
...
Summary statistics (median + MAD over a sample set) cost a handful of
allocations per call — eight is the gated ceiling.
<!-- /soothfast:claim -->
...
<p align="center">
  <em>soothfast</em> — Old English <em>sōþfæst</em>: "fixed in truth." MIT licensed.
</p>
