...
/// Resolve nav groups to template data. Explicit nav wins; pages it names
/// but the build didn't find are errors (a dead sidebar link is drift).
pub fn build(cfg: &SiteConfig, pages: &[PageMeta]) -> Result<Value, String> {
    if cfg.nav.is_empty() {
        return Ok(auto(pages));
    }
    ...
    for group in &cfg.nav {
        ...
        for entry in &group.pages {
            ...
            let page = pages
                .iter()
                .find(|p| p.src == src)
                .ok_or_else(|| format!("nav group {:?} lists missing page {src:?}", group.title))?;
...
mod tests {
    ...
    fn explicit_nav_resolves_and_rejects_missing() {
        ...
        let nav = build(&cfg, &pages).unwrap();
    ...
    fn auto_nav_groups_root_then_dirs() {
        ...
        let nav = build(&SiteConfig::default(), &pages).unwrap();
