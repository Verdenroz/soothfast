...
pub struct BuildInput {
    ...
    /// What a nav entry naming an absent page does. `docs build` fails on
    /// one; `docs serve` reports it and carries on.
    pub missing_nav_page: nav::Missing,
}
...
pub struct BuildReport {
    ...
    pub assets: usize,
    /// Non-fatal problems, for the caller to print.
...
pub fn build(input: &BuildInput) -> Result<BuildReport, String> {
    ...
    let (nav_data, warnings) = nav::build(cfg, &metas, input.missing_nav_page)?;
...
mod tests {
    ...
    fn end_to_end_build_smoke() {
        ...
        let input = BuildInput {
            missing_nav_page: nav::Missing::Fail,
            root: root.clone(),
